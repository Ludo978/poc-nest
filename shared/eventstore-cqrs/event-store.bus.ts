...
export class EventStoreBus {
  ...
  constructor(
    ...
    const persistentSubscriptions = config.subscriptions.filter((sub) => sub.type === EventStoreSubscriptionType.Persistent);
  ...
  async subscribeToPersistentSubscriptions(
    ...
    this.persistentSubscriptions = await Promise.all(
      subscriptions.map(async (subscription) => this.subscribeToPersistentSubscription(
        subscription.stream,
  ...
  async createMissingPersistentSubscriptions(
    ...
    try {
      await Promise.all(
        subscriptions.map(async (subscription) => this.eventStore
          .getConnection()
  ...
  subscribeToCatchUpSubscriptions(subscriptions: ESCatchUpSubscription[]) {
    ...
    this.catchupSubscriptions = subscriptions.map((subscription) => this.subscribeToCatchupSubscription(subscription.stream));
  }
  ...
  get allCatchUpSubscriptionsLive(): boolean {
    ...
    return (
      initialized
      && this.catchupSubscriptions.every((subscription) => !!subscription && subscription.isLive)
    );
  }
  ...
  get allPersistentSubscriptionsLive(): boolean {
    ...
    return (
      initialized
      && this.persistentSubscriptions.every((subscription) => !!subscription && subscription.isLive)
    );
  }
  ...
  async publishAll(events: IEvent[], stream?: string) {
    try {
      await this.eventStore.getConnection().appendToStream(
        stream,
        -2,
        (events || []).map((event: IEvent) => jsonEvent<JSONEventType>({
          type: event.constructor.name,
          data: JSON.parse(JSON.stringify(event)).data,
        })),
      );
  ...
  subscribeToCatchupSubscription(stream: string): ExtendedCatchUpSubscription {
    ...
    try {
      var resolved = this.eventStore
        ...
        .on('confirmation', () => this.onLiveProcessingStarted(resolved as ExtendedCatchUpSubscription));
      return resolved;
    } catch (err) {
      this.logger.error(err.message, err.stack);
    }
  ...
  onDropped(
    ...
    if (subscriptionName) { this.reSubscribeToPersistentSubscription(stream, subscriptionName); }
  }
  ...
  reSubscribeToPersistentSubscription(
    ...
    if (
      ...
      setTimeout(
        (stream, subscriptionName) => this.subscribeToPersistentSubscription(stream, subscriptionName),
        3000,
        stream,
        subscriptionName,
      );
